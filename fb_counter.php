...
// Check if user is event admin and has F&B Counter permission
if ($_SESSION['user_type'] !== 'event_admin' || !in_array('F&B Counter', $_SESSION['permissions'])) {
...
// Handle F&B distribution submission
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($check_result->num_rows === 0) {
        ...
        $sql = "INSERT INTO fb_logs (event_id, participant_id, item_name, quantity) 
                VALUES ($event_id, '$participant_id', '$item_name', $quantity)";
        if ($conn->query($sql)) {
            $success = "F&B distribution recorded successfully";
        } else {
...
$sql = "SELECT * FROM fb_logs WHERE event_id = $event_id ORDER BY distribution_time DESC LIMIT 10";
$recent_distributions = $conn->query($sql);

// Get total distributions for today
$today = date('Y-m-d');
$sql = "SELECT item_name, SUM(quantity) as total 
        FROM fb_logs 
        WHERE event_id = $event_id 
        AND DATE(distribution_time) = '$today'
        GROUP BY item_name";
$today_stats = $conn->query($sql);

// Define available F&B items
$fb_items = [
    'Lunch Box - Veg',
    'Lunch Box - Non-Veg',
    'Coffee',
...
<head>
    ...
    <title>F&B Counter Management - Event Management System</title>
                ...
                <div class="flex justify-between h-16">
                    <div class="flex">
                        <div class="flex-shrink-0 flex items-center">
                            <a href="admin_dashboard.php" class="text-xl font-semibold">F&B Counter Management</a>
                        </div>
                    </div>
        ...
        <main class="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
            <div class="px-4 py-6 sm:px-0">
                ...
                <div class="bg-white shadow rounded-lg mb-6">
                    <div class="px-4 py-5 sm:p-6">
                        <h3 class="text-lg font-medium text-gray-900 mb-4">Record F&B Distribution</h3>
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    <?php while($dist = $recent_distributions->fetch_assoc()): ?>
                                        <tr>
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                                <?php echo $dist['quantity']; ?>
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                                <?php echo date('M j, Y H:i:s', strtotime($dist['distribution_time'])); ?>
                                            </td>
                                        </tr>
                                    <?php endwhile; ?>
                                </tbody>
